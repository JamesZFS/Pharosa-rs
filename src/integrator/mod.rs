...
pub trait Integrator {
    ...
    fn render<C, S>(film: &mut Film, camera: &Camera<C>, scene: &Scene, sampler: &mut S)
...
pub trait SampleIntegrator {
    ...
    fn Li(ray: Ray, scene: &Scene, sampler: &mut impl Sampler) -> Spectrum;
}

impl<T> Integrator for T where T: SampleIntegrator {
    fn render<C, S>(film: &mut Film, camera: &Camera<C>, scene: &Scene, sampler: &mut S) where C: CameraInner, S: Sampler {
